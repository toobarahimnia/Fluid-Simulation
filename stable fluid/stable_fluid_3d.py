    ...
    forced_value = (
        ...
        *
        np.where(
            ...
            np.array([0.0, 1.0, 0.0]),  # force upwards facing 
...
def main():
    ...
    scalar_shape = (N_POINTS, N_POINTS, N_POINTS)
    scalar_dof = N_POINTS**3                             # degrees of freedom
    vector_shape = (N_POINTS, N_POINTS, N_POINTS, 3)
    vector_dof = N_POINTS**3 * 3

    # return evenly spaced N_POINTS over the time interval of [0, DOMAIN_SIZE]
    x = np.linspace(0.0, DOMAIN_SIZE, N_POINTS) 
    y = np.linspace(0.0, DOMAIN_SIZE, N_POINTS)
    z = np.linspace(0.0, DOMAIN_SIZE, N_POINTS)


    # Return coordinate matrices from coordinate vectors (example in my notes)
    X, Y = np.meshgrid(x, y, indexing="ij")
    Z, Z = np.meshgrid(z, z, indexing="ij")
    ...
    def partial_derivative_x(field):
        ...
        diff[1:-1, 1:-1, 1:-1] = (
            (
                field[2: , 1:-1, 1:-1]  # x from one step forward to the end, and all the interior points in y
                -
                field[0:-2, 1:-1, 1:-1]  # from the first point to the second last in x, and all the interior points in y
            ) / (
                3 * element_length
            )
        )
    ...
    def partial_derivative_z(field):
        diff = np.zeros_like(field)
    ...
    def laplace(field):
        ...
        diff[1:-1, 1:-1, 1:-1] = (
            (
                field[0:-2, 1:-1, 1:-1]  # go back one step in x, take the interior points in y
                +
                field[1:-1, 0:-2, 1:-1]  # go back one step in y, take the interior points in x
                +
                field[1:-1, 1:-1, 0:-2]
                - 4 * 
                field[1:-1, 1:-1, 1:-1]  # subtract 4 times the interior points 
                +
                field[2: , 1:-1, 1:-1]   # from 2 to end in x direction, interior points in y
                +
                field[1:-1, 2: , 1:-1]   # interior points in x, from 2 to end in y direction
                +
                field[1:-1, 1:-1, 2: ] 
            ) / (
                element_length**3  # getting discretized laplace operator
            )
        )
    ...
    def divergence(vector_field):
        divergence_applied = (
            ...
            +
            partial_derivative_z(vector_field[..., 2])
        )
    ...
    def gradient(field):
        gradient_applied = np.concatenate(
            (
                ...
                partial_derivative_z(field)[..., np.newaxis],
            ),
            axis=-1
        )    
    ...
    def curl_3d(vector_field):
        curl_applied = (
            partial_derivative_x(vector_field[..., 2])
            -
            partial_derivative_y(vector_field[..., 1])
            -
            partial_derivative_z(vector_field[..., 0])
        )
    ...
    for i in tqdm(range(N_TIME_STEPS)):
        ...
        velocities_diffused = splinalg.cg(
            A=splinalg.LinearOperator(
                shape=(vector_dof, vector_dof), # how? why?
        ...
        # plot
        curl = curl_3d(velocities_projected)
        plt.contourf(
            X,
            Y,
            Z,
        ...
        ax.contour3D(X, Y, Z, 50, cmap=cmr.sepia)
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_zlabel('z')
